...
ruff, mypy, pytest, `.gitignore`, `.env.example`, README stub,
`CONTRIBUTING.md` documenting the worktree convention.

**Acceptance criteria.**
- `uv sync --all-extras` succeeds from a clean clone.
- `make quick` (ruff + mypy) returns clean on the empty `src/auto_research/`.
- `make check` (= `make quick` + pytest) passes with an empty suite.
